...
document.addEventListener('DOMContentLoaded', () => {
  ...
  fileUploadContainers.forEach((container) => {
      ...
      if (isImageFile && fileData.status !== 'error') {
        ...
        altBox.innerHTML = [
          ...
          '  <dd><input type="text" name="' +
            ...
            '" id="' +
            altId +
            '" class="krds-input small" placeholder="입력해 주세요"/></dd>',
          '</dl>',
        ].join('');
    ...
    function updateFileCount() {
      ...
      if (maxCount) {
        maxCount.innerHTML = `<span class="current">${currentCount}개</span> / ${config.maxFiles}개`;
      }
...
(function () {
  ...
  function openPopup(trigger, popup) {
    ...
    if (popWrap) {
      ...
      setTimeout(() => {
        popWrap.focus({ preventScroll: true });
      }, 100);
    } else {
...
function setActiveTabBar() {
	...
	const buttons = document.querySelectorAll("nav.tab_bar .nav_list li .hf_btn[href]");
	...
	buttons.forEach((btn) => {
		...
		console.log("href:", href, "linkPath:", linkPath);
		...
		if (
			...
			linkPath.endsWith(currentPath.split("/").slice(-3).join("/"))
		) {
...
document.addEventListener("DOMContentLoaded", setActiveTabBar);

document.addEventListener('DOMContentLoaded', function () {
  const visualSwiper = new Swiper('.popupzon .visual-slider', {
  loop: false,
  rewind: true,
  initialSlide: 0,
  centeredSlides: true,
  ...
  spaceBetween: 8,
    ...
    navigation: {
      prevEl: '.popupzon .swiper-prev-custom',
      nextEl: '.popupzon .swiper-next-custom',
    },

    pagination: {
      el: '.popupzon .swiper-pagination-custom',
      clickable: true,
  ...
  const toggleButton = document.querySelector('.popupzon .swiper-toggle-custom');
  let isPlaying = true;
...
document.addEventListener('DOMContentLoaded', function () {
  const subVal = document.querySelector('.sub_val');
  const currentEl = document.querySelector('.sub_val .slide-count .current');
  const totalEl = document.querySelector('.sub_val .slide-count .total');
  const totalSlides = document.querySelectorAll('.sub_val .slide_sub_area .swiper-slide').length;

  if (totalEl) {
    totalEl.textContent = totalSlides;
  }

  const visualSwiper = new Swiper('.sub_val .slide_sub_area', {
    loop: false,
    rewind: true,
    effect: 'fade',
    fadeEffect: {
    ...
    navigation: {
      prevEl: '.sub_val .swiper-prev-custom',
      nextEl: '.sub_val .swiper-next-custom',
    },
  ...
  const toggleButton = document.querySelector('.sub_val .swiper-toggle-custom');
  let isPlaying = true;
...
document.addEventListener('DOMContentLoaded', function () {
  const slider = document.querySelector('.data_list_area');

  if (!slider) return;

  const slides = Array.from(slider.querySelectorAll('.swiper-slide'));
  const activeIndex = slides.findIndex(function (slide) {
    return slide.querySelector('.cont.active');
  });

  const dataListSwiper = new Swiper(slider, {
    slidesPerView: 4,
    slidesPerGroup: 1,
    spaceBetween: 0,
    loop: false,
    ...
    pagination: {
      el: '.loan_data_info_area.slide .swiper-pagination-custom',
      clickable: true,
      renderBullet: function (index, className) {
        return `<button type="button" class="${className}" aria-label="${index + 1}번 슬라이드로 이동"></button>`;
      },
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const mainValSwiper = new Swiper('.main_slide .swiper', {
    slidesPerView: 1,   // 1개씩 노출
    spaceBetween: 0,
    loop: true,        // 루프 비활성화
...
/* ==========================================================================
   [2026 신규] 모바일 메인 — 시안 : 이미지/MB/00. 메인페이지/*.jpg
   - 기존 초기화 코드는 건드리지 않고 아래 블록만 추가.
   - 대상 요소가 없는 페이지에서는 아무 것도 하지 않는다.
   ========================================================================== */
document.addEventListener('DOMContentLoaded', function () {

  /* 1. 히어로 배너 (1장씩, 자동 루프) */
  if (document.querySelector('.nmain_hero .swiper')) {
    new Swiper('.nmain_hero .swiper', {
      slidesPerView: 1,
      spaceBetween: 0,
      loop: true,
      autoplay: { delay: 4000, disableOnInteraction: false }
      /* 도트는 배너 PNG 안에 그려져 있어 pagination 을 쓰지 않는다 */
    });
  }

  /* 2. 맞춤 추천 상품 (카드 가로 스크롤, 좌우 여백 16 유지) */
  if (document.querySelector('.nmain_promo .swiper')) {
    new Swiper('.nmain_promo .swiper', {
      slidesPerView: 'auto',
      spaceBetween: 12,
  ...
  /* 3. 신청 진행 카드 안의 3단계 미니 스텝퍼 (도트로 페이지 이동) */
  document.querySelectorAll('.nmain_step_box').forEach(function (box) {
    var pages = box.querySelectorAll('[data-step-page]');
    var dots = box.querySelectorAll('.nmain_step_dots button');
  ...
  /* 4. 원형 게이지 : data-pct 값을 CSS 변수로 넘긴다 */
  document.querySelectorAll('.nmain_gauge[data-pct]').forEach(function (g) {
    g.style.setProperty('--p', g.getAttribute('data-pct') + '%');
  });

  /* 5. 하단 탭바 바텀시트는 include.js 의 data-pop 엔진이 처리한다(중복 구현하지 않음) */

  /* 6. 시트 안에서 다음 시트를 열 때는 현재 시트를 닫는다 (드릴다운, 쌓이지 않게)
        예) 업무구분 > 주택담보대출 -> 증명서 유형 */
  document.addEventListener('click', function (e) {
    var link = e.target.closest('.com_pop_wrap [data-pop]');
    if (!link) return;
    var cur = link.closest('.com_pop_wrap');
    if (!cur) return;
    setTimeout(function () {          // include.js 가 다음 시트를 연 뒤에 닫는다
      var cont = cur.querySelector('.com_pop_cont');
